...
const schema_postgres = `-- Paste full schema_postgres.sql content here from previous read
...
async function setupDB() {
  try {
    ...
    await pool.query(schema_postgres.split('-- Insert')[0]); // Drop + CREATE

    console.log('📦 Inserting data...');
    const inserts = schema_postgres.split('-- Insert')[1];
    ...
    console.log(`📊 Branches: ${branches.rows[0].count}, Users: ${users.rows[0].count}`);
